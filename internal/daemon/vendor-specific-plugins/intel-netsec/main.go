...
func (vsp *intelNetSecVspServer) finishVfSetup(ifName string, vfID int) (string, string, error) {
	...
	vfIfName, err := vspnetutils.WaitForNetDevReady(vsp.platform, pcieAddr, IntelWaitForVfSetupTimeout)
	if err != nil {
		vsp.log.Error(err, "Error getting netdev name from PCIe address", "PCIeAddress", pcieAddr)
		return "", "", err
	}

	vfLink, err := vspnetutils.WaitForLinkReady(vfIfName, IntelWaitForVfSetupTimeout)
	if err != nil {
